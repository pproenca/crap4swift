...
@Suite("CRAP Formula")
struct CrapScoreTests {
    @Test("Full coverage equals complexity")
    func fullCoverageEqualsCc() {
    ...
    @Test("Zero coverage inflates score")
    func zeroCoverage() {
    ...
    @Test("Complexity 1 and full coverage")
    func complexity1FullCoverage() {
        // Simplest possible: CC=1, 100% coverage → CRAP = 1
        assertCrapEquals(complexity: 1, coverage: 100.0, expected: 1.0)
    }

    @Test("High complexity and low coverage")
    func highComplexityLowCoverage() {
    ...
    @Test("Formula verification samples")
    ...
    private func assertCrapEquals(complexity: Int, coverage: Double, expected: Double, accuracy: Double = 0.001) {
        #expect(abs(crapScore(complexity: complexity, coveragePercent: coverage) - expected) <= accuracy)
    }
}
